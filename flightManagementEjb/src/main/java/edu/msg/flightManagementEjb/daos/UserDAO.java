...
public class UserDAO {
	...
	public void deleteUser(User user) {
		try {
			...
			user.ListFlights(new ArrayList<>());
	...
	public List<User> getUsersByTypeAndCompany(String type, Company company, boolean status) {
		try {
			TypedQuery<User> query = entityManager.createQuery(
					"SELECT u FROM User u WHERE u.type = :type and u.company = :company and u.status = :status",
					User.class);
			query.setParameter("type", type);
			...
			logger.error("Users retrieval by type and company failed", e);
			throw new DaoException("Users retrieval by type and company failed",e);
		}
	}

	public List<User> getUsersByCompany(Company company) {
		try {
			TypedQuery<User> query = entityManager.createQuery("SELECT u FROM User u WHERE u.company = :company",
					User.class);
			...
			logger.error("Users retrieval by company failed", e);
			throw new DaoException("Users retrieval by company failed",e);
		}
	}
